...
export default function InfoSection({ business }: { business?: Business }) {
  ...
  useEffect(() => {
    ...
    fetch(url)
      .then((r) => r.json())
      .then((data) => { if (data.hours) setHours(data.hours); })
      .catch(() => {});
  }, [slug]);
        ...
        <div className="info__grid">
          ...
          <div className="info__block">
            ...
            <div className="contacts">
              <a href={`tel:${business?.phone ?? "+390308860239"}`} className="contact-card">
                ...
                <div className="contact-info"><span className="contact-label">Telefono</span><span className="contact-value">{business?.phone ?? "+39 030 886 0293"}</span></div>
              ...
              <a href={`https://wa.me/${(business?.wa_phone ?? "390308860293").replace(/\D/g, "")}`} target="_blank" rel="noopener noreferrer" className="contact-card">
                ...
                <div className="contact-info"><span className="contact-label">WhatsApp</span><span className="contact-value">Scrivici su WhatsApp</span></div>
                <svg className="contact-arrow" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M5 12h14M12 5l7 7-7 7"/></svg>
              </a>
              <a href="https://maps.app.goo.gl/Ufs9Xwda54Nwj44G6" target="_blank" rel="noopener noreferrer" className="contact-card">
                ...
                <div className="contact-info"><span className="contact-label">Indirizzo</span><span className="contact-value">{business?.address ?? "Via Esempio 1, Città"}</span></div>
                <svg className="contact-arrow" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M5 12h14M12 5l7 7-7 7"/></svg>
              </a>
            </div>
            <div className="delivery-note">
              <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg>
              Consegna a domicilio nel raggio di <strong>{business?.radius_km ?? 5} km</strong>. Ritiro in pizzeria sempre disponibile.
            </div>
          </div>
