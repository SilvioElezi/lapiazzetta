...
export default function CheckoutDrawer() {
  ...
  const placeOrder = async () => {
    ...
    try {
      const res = await fetch("/api/order", {
        ...
        body: JSON.stringify({ clientName: clientName.trim(), phone: phone.trim(), address: address.trim(), lat, lng, items: cart.map((i: any) => ({ id: i.id, name: i.name, qty: i.qty, price: i.price })), total }),
      });
      const data = await res.json();
      setOrderId(data.id);
      setStep("done");
  ...
  const resetAndClose = () => {
    ...
    setLat(null); setLng(null); setOrderId("");
  };
    ...
    <>
      ...
      <aside className={`drawer${open ? " drawer--open" : ""}`}>
        ...
        {step === "form" && <>
          ...
          <div className="drawer__body">
            <div className="form">
              <label className="field"><span>Nome e cognome *</span>
                <input type="text" placeholder="Mario Rossi" value={clientName} onChange={(e) => setClientName(e.target.value)} autoComplete="name" />
              </label>
              <label className="field"><span>Telefono *</span>
                <input type="tel" placeholder="+39 333 123 4567" value={phone} onChange={(e) => setPhone(e.target.value)} autoComplete="tel" />
              </label>
              <div className="field"><span>Indirizzo *</span>
                <div className="addr-row">
                  <input type="text" placeholder="Via Roma 12, Città" value={address} onChange={(e) => { setAddress(e.target.value); setLat(null); setLng(null); }} />
                  <button type="button" className="gps-btn" onClick={getGPS} disabled={locating}>{locating ? "⏳" : "📍"}</button>
                </div>
          ...
          <div className="drawer__foot">
            <button className="btn-primary" onClick={placeOrder} disabled={placing || !clientName || !phone || !address}>
        ...
        {step === "done" && (
          <div className="done">
            ...
            <h2 className="done__title">Ordine ricevuto!</h2>
